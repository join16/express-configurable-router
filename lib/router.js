...
Router.prototype.param = function(paramName, config) {
  var _config = utils._.assign({}, this._defaultConfig, config);
  ...
  this._expressRouter.param(paramName, function(req, res, next, value) {
    utils._.forEach(pluginParamHandlers, function(paramHandler) {
      var parsedValue = paramHandler(req, res, value);
...
Router.prototype.config = function(config) {
  utils._.assign(this._defaultConfig, config);
};

Router.prototype.getRouteTables = function() {
  var routeTableResults = {};
  
  utils._.forEach(this._plugins, function(plugin, name) {
    if (utils._.isFunction(plugin.routeTableResult)) {
      routeTableResults[name] = plugin.routeTableResult;
    }
  });
  
  var routeTable = utils._.cloneDeep(this._routeTables);
  
  return utils._.map(routeTable, function(routeTable) {
    var table = {
      url: routeTable.url,
    ...
    utils._.forOwn(routeTable.config, function(value, key) {
      ...
      if (routeTableResults[key]) {
        table.config[key] = routeTableResults[key](value);
      } else {
