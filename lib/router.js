...
function Router(options) {
  ...
  if (utils._.isObject(options.params)) {
    this._expressRouter.params = options.params;
  }
}
...
Router.prototype.route = function(prefix, routingFunc) {
  var childRouter = new Router({
    ...
    params: utils._.cloneDeep(this._expressRouter.params)
  });
...
Router.prototype._addRoute = function(url, method, config) {
  ...
  // add handler
  var handlers = utils._.isFunction(_config.handler) ?
    [_config.handler] :
    _config.handler;
  if (!utils._.isArray(handlers)) {
