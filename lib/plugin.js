 ...
 * @param {Function} [params.comments] comments parser function for routeTables
 * @constructor
 */
function Plugin(params) {
  ...
  if (utils._.isFunction(params.routeTableResult)) {
    this.routeTableResult = params.routeTableResult.bind(this);
  ...
  if (utils._.isFunction(params.comments)) {
    this.comments = params.comments.bind(this);
  }
...
Plugin.prototype.getParamHandlerMiddleware = function(pluginConfig) {
  ...
  return function(req, res, key, value) {
    return paramHandler(pluginConfig, options, req, res, key, value);
  };
};
