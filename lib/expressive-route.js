...
ExpressiveRoute.prototype.plugin = function(pluginParams, options) {
  ...
  } else if (utils._.isObject(pluginParams)) {
    plugin = new Plugin(pluginParams);
...
ExpressiveRoute.prototype.getExpressRouter = function() {
  if (utils._.isNil(this.router)) {
    throw new Error('no route has registered');
  }
  
  return this.router._expressRouter;
};